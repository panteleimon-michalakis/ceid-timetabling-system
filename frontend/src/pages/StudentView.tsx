...
export default function StudentView() {
  ...
  // Load all published timetables
  useEffect(() => {
    api.get<Timetable[]>('/timetables').then(r => {
      // Backend already filters by role (STUDENT sees only PUBLISHED)
      const all = r.data;
      setTimetables(all);
      // Default: first semester timetable; if none, first exam
  ...
  function TtItem({ t }: { t: Timetable }) {
    ...
    return (
      <div className="tt-option" onClick={() => setSelectedTtId(t.id)}
        style={{
