...
replace (word-boundary/regex ή guard «αν ήδη …ς»).
**Impact (ΑΝΑΘΕΩΡΗΜΕΝΟ από BL-11):** ΟΧΙ πλέον «καθαρά εμφανισιακό». Το corrupted
`snapshot_teachers_text` (το BL-8 τρέχει μέσα στο `normalizeTeachersTextForDto` του stamper)
**μπλοκάρει** το teacher snapshot-first του BL-11: audit βρήκε **307/4815 (6.4%)** assignments με
teacher-key drift (`ΗΛΙΑΣ|Α` live vs `ΗΛΙΑΣΣΣ|Α` snapshot) → snapshot-first teacher θα εισήγαγε
TEACHER_CONFLICT regression. Γι' αυτό το BL-11 άφησε teacher **live** (carve-out). Fix = idempotent
cleaning **+ clean re-stamp των frozen snapshot rows** (~307 αναθέσεις) — **HIGH-RISK (αγγίζει
frozen δεδομένα), απαιτεί auditor sign-off** + DB backup. Ξεκλειδώνει το πλήρες teacher BL-11
(γύρισμα 1 γραμμής + το format-consistency test ως gate). Tier: 🟣/🔵 (αναβαθμισμένο από 🟢).
...
**[BL-11] Snapshot-first hard validation — ✅ PARTIAL (6c00af3· teacher deferred → BL-8)**
...
**PARTIAL (6c00af3):** Ο analysis path (`SolverService.buildPlacedSolution` +
`ValidationEngineService.toView`) διαβάζει πλέον snapshot-first τα hard-relevant πεδία:
courseCode/Name, **studyYear**, courseType, semester, room (**code/type**/capacity), timeslot
(**day/hour**). Ξεχωριστοί helpers (`buildSnapshotRoom`/`buildSnapshotTimeSlot`) → solve path
(`buildLessons` + shared mappers) **ΑΝΕΠΑΦΟ**, solver baseline αμετάβλητος. Live σκόπιμα:
expectedStudents (soft), semesterType (inert), exam-prefs (soft/αδρανή). **Teacher deferred →
[BL-8]:** audit έδειξε ότι το `snapshot_teachers_text` είναι corrupted (307/4815 = **6.4%** key
drift από το BL-8 «ςςς» bug), άρα teacher keys/names μένουν **live** (snapshot-first θα εισήγαγε
TEACHER_CONFLICT regression). Ο `snapshotTeacherKeys` helper + format-consistency test μένουν ως
έτοιμο hook (γύρισμα = 1 γραμμή μετά το BL-8 fix + clean re-stamp). Tests: `SnapshotFirstHelpersTest`
(2) + `SnapshotFirstHardValidationTest` (2· studyYear + room immutability)· full suite 204/204. Βλ.
THESIS_NOTES [6c00af3].
